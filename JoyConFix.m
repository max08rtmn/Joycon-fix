...
/*
 ...
 What this tweak does:
   ...
   2. Returns the physical button objects directly from buttonA/B/X/Y and
      shoulder getters. This matters for emulators that attach handlers to the
      returned button object instead of polling value/isPressed later.
 ...
 It does not touch axes. The Gemini stick rotation was only a clue that
 GCController-level hooks are effective inside LiveContainer.
*/
...
static id JCFRemappedGetter(id gamepad, SEL selector, NSString *role, JCFIdGetterIMP original) {
    ...
    if (source) {
        /*
         Return the real physical source object. This is the important part for
         emulators: if MelonX installs valueChangedHandler/pressedChangedHandler
         on buttonA/buttonB/buttonX/buttonY, those handlers must be attached to
         four distinct physical buttons, not to the single broken object that
         iOS exposes to the app layer in separated fake-Joy-Con mode.
        */

...
static JCFSetGamepadButtonStateC gOrigSetGamepadButtonStateDirect = NULL;
typedef void (*JCFMSHookFunctionType)(void *symbol, void *replace, void **result);
static JCFMSHookFunctionType gMSHookFunction = NULL;
static BOOL gDidDirectHook = NO;
...
static void JCFSendBridgeState(void *gamepad, NSString *semantic, BOOL pressed, const char *reason) {
    JCFSetGamepadButtonStateC sender = gOrigSetGamepadButtonStateDirect ?: gOrigSetGamepadButtonState;
    if (!gamepad || !sender || !semantic) {
        return;
    }
    ...
    NSLog(@"[JoyConFixBridgeInject] send %@ id=%ld pressed=%d reason=%s",
          semantic, buttonId, pressed, reason ?: "unknown");
    sender(gamepad, buttonId, pressed);
}

static void JCFReplayPhysicalStates(void *gamepad, const char *reason) {
    if (!gamepad || !(gOrigSetGamepadButtonStateDirect ?: gOrigSetGamepadButtonState) || !gPhysicalPressed) {
        return;
    }
    for (NSString *semantic in JCFAllSemantics()) {
        BOOL pressed = [gPhysicalPressed[semantic] boolValue];
        JCFSendBridgeState(gamepad, semantic, pressed, reason);
    }
}
...
static void JCFRememberPhysicalState(NSString *semantic, BOOL pressed) {
    ...
    if (gLastRyujinxGamepad && (gOrigSetGamepadButtonStateDirect ?: gOrigSetGamepadButtonState)) {
...
static void JCFBridgeSetGamepadButtonState(void *gamepad, long buttonId, bool pressed) {
    ...
    if (gSawManagedJoyConState && JCFBridgeIdIsManaged(buttonId)) {
        NSLog(@"[JoyConFixBridgeInject] suppress original id=%ld pressed=%d and replay physical states",
              buttonId, pressed);
...
static void JCFDirectSetGamepadButtonState(void *gamepad, long buttonId, bool pressed) {
    if (gamepad) {
        gLastRyujinxGamepad = gamepad;
    }
    NSLog(@"[JoyConFixDirectHook] hit id=%ld pressed=%d sawJoyCon=%d", buttonId, pressed, gSawManagedJoyConState);
    if (gSawManagedJoyConState && JCFBridgeIdIsManaged(buttonId)) {
        NSLog(@"[JoyConFixDirectHook] suppress direct id=%ld pressed=%d and replay physical states",
              buttonId, pressed);
        JCFReplayPhysicalStates(gamepad ?: gLastRyujinxGamepad, "direct-suppress-replay");
        return;
    }
    if (gOrigSetGamepadButtonStateDirect) {
        gOrigSetGamepadButtonStateDirect(gamepad, buttonId, pressed);
    } else if (gOrigSetGamepadButtonState) {
        gOrigSetGamepadButtonState(gamepad, buttonId, pressed);
    }
}

static void JCFLoadMSHookFunction(void) {
    if (gMSHookFunction) {
        return;
    }
    gMSHookFunction = (JCFMSHookFunctionType)dlsym(RTLD_DEFAULT, "MSHookFunction");
    if (!gMSHookFunction) {
        ...
        if (handle) {
            gMSHookFunction = (JCFMSHookFunctionType)dlsym(handle, "MSHookFunction");
        }
    }
    NSLog(@"[JoyConFixDirectHook] MSHookFunction=%p", gMSHookFunction);
}

static void JCFInstallDirectHook(void) {
    if (gDidDirectHook) {
        return;
    }
    JCFLoadMSHookFunction();
    if (!gMSHookFunction) {
        NSLog(@"[JoyConFixDirectHook] no MSHookFunction available");
        return;
    }

    const char *symbols[] = {
        "set_gamepad_button_state",
        "_set_gamepad_button_state",
        "$s6MeloNX13RyujinxBridgeC21setGamepadButtonState_8buttonId7pressedySvSg_SiSbtFZTf4nnnd_n",
        "_$s6MeloNX13RyujinxBridgeC21setGamepadButtonState_8buttonId7pressedySvSg_SiSbtFZTf4nnnd_n"
    };
    ...
    for (NSUInteger i = 0; i < sizeof(symbols) / sizeof(symbols[0]); i++) {
        ...
        gMSHookFunction(symbol, (void *)JCFDirectSetGamepadButtonState, (void **)&gOrigSetGamepadButtonStateDirect);
        gDidDirectHook = YES;
        NSLog(@"[JoyConFixDirectHook] hooked %s symbol=%p original=%p",
              symbols[i], symbol, gOrigSetGamepadButtonStateDirect);
        return;
    }

    NSLog(@"[JoyConFixDirectHook] no direct symbol found");
}

static void JCFRebindSection(const struct mach_header_64 *header,
                             intptr_t slide,
                             struct section_64 *section,
                             struct symtab_command *symtabCommand,
                             struct dysymtab_command *dysymtabCommand,
                             struct segment_command_64 *linkeditSegment) {
    if (!section || !symtabCommand || !dysymtabCommand || !linkeditSegment) {
        return;
    }
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (strcmp(symbolName, "_set_gamepad_button_state") != 0) {
            continue;
        }

        if (!gOrigSetGamepadButtonState) {
            gOrigSetGamepadButtonState = (JCFSetGamepadButtonStateC)bindings[i];
        }
        ...
        if (kr == KERN_SUCCESS) {
            bindings[i] = (void *)JCFBridgeSetGamepadButtonState;
            NSLog(@"[JoyConFixBridgeInject] rebound _set_gamepad_button_state in %s/%s original=%p",
                  section->segname, section->sectname, gOrigSetGamepadButtonState);
        } else {
            NSLog(@"[JoyConFixBridgeInject] failed to rebind _set_gamepad_button_state kr=%d", kr);
        }
    }
}

static void JCFRebindImage(const struct mach_header *mh, intptr_t slide) {
        ...
        if (command->cmd == LC_SEGMENT_64) {
            ...
            for (uint32_t j = 0; j < segment->nsects; j++) {
                ...
                if (type == S_LAZY_SYMBOL_POINTERS || type == S_NON_LAZY_SYMBOL_POINTERS) {
                    JCFRebindSection(header, slide, &section[j], symtabCommand, dysymtabCommand, linkeditSegment);
                }
            }
        }
        cursor += command->cmdsize;
...
static void JCFInstallBridgeRebind(void) {
    uint32_t imageCount = _dyld_image_count();
    for (uint32_t i = 0; i < imageCount; i++) {
        const struct mach_header *header = _dyld_get_image_header(i);
        intptr_t slide = _dyld_get_image_vmaddr_slide(i);
        JCFRebindImage(header, slide);
    }
}
...
static void JCFInit(void) {
    @autoreleasepool {
        NSLog(@"[JoyConFixReroute] loaded 2026-06-29 direct-hook exact-button-match");
        ...
        JCFWriteLoadProof();
        JCFInstallBridgeRebind();
        JCFInstallDirectHook();

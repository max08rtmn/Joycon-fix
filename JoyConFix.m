...
static BOOL JCFPatchOneJSONFile(NSString *path, BOOL forceRoot) {
    ...
    if (!object) {
        ...
        NSLog(@"[JoyConBridge] ONCE-STATUS config missing path=%@", path);
        return NO;
    }
    ...
    if (!changed) {
        ...
        NSLog(@"[JoyConBridge] ONCE-STATUS config unchanged path=%@", path);
        return YES;
    }

    BOOL ok = JCFWriteJSONToFile(object, path);
    NSLog(@"[JoyConNintendoOff] patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    NSLog(@"[JoyConBridge] ONCE-STATUS config patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    return YES;
}
...
static void JCFPatchMeloNXConfigFiles(void) {
    ...
    NSLog(@"[JoyConBridge] ONCE-STATUS config attempt=%lu home=%@", (unsigned long)gJCFConfigPatchAttempts, home);
    ...
    if (!gJCFConfigPatchSucceeded) {
        ...
        NSLog(@"[JoyConBridge] ONCE-STATUS config retry_pending");
    }
}
...
static id JCFPhysicalButtons(id self, SEL _cmd) {
    ...
    if (!loggedPatchTrigger) {
        ...
        NSLog(@"[JoyConBridge] ONCE-STATUS config patch triggered from physical buttons");
    }
...
static void JCFBridgeProfileForController(id controller, id profile, BOOL micro) {
    ...
    if (!loggedProfilePatchTrigger) {
        ...
        NSLog(@"[JoyConBridge] ONCE-STATUS config patch triggered from profile bridge micro=%@", micro ? @"YES" : @"NO");
    }
    JCFPatchMeloNXConfigFiles();
...
static void JCFInstall(void) {
    @autoreleasepool {
        NSLog(@"[JoyConBridge] ONCE-STATUS 2026-06-29-controller-triggered-config");
        NSLog(@"[JoyConNintendoOff] LOAD MARKER 2026-06-29-once-status");

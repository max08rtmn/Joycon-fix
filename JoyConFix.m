...
/*
 JoyConFix.m - call-stack read probe, identity-neutral
 ...
 It proves that the exact dylib is loaded, logs physical button reads when
 MeloNX asks GameController for value/isPressed, and logs a short call stack
 on button-down. The stack is needed because the GameController values are
 already correct; the remaining bug is in MeloNX/Ryujinx after the read.
...
static NSString *JCFStackSummary(void) {
    ...
    for (NSUInteger i = 2; i < symbols.count && kept.count < 10; i++) {
        NSString *line = symbols[i];
        if ([line containsString:@"JoyConFix"] ||
            [line containsString:@"Foundation"] ||
            [line containsString:@"CoreFoundation"]) {
            continue;
        }
        [kept addObject:line];
    }
...
static void JCFLogReadIfChanged(id button, NSString *kind, float value, BOOL pressed, BOOL pressedKnown) {
    ...
    if (changed && active) {
        NSLog(@"[JoyConSure] read %@ %@ value=%.3f pressed=%@ ptr=%p",
              ...
              button);
        if (value > 0.10f) {
            NSLog(@"[JoyConSure] stack %@ %@", JCFLabelForButton(button), JCFStackSummary());
        }
    }
}
...
static void JCFInstall(void) {
    @autoreleasepool {
        NSLog(@"[JoyConSure] LOAD MARKER 2026-06-28-call-stack-read-probe");

...
static IMP gOrigGamepadLeftTrigger;
static IMP gOrigGamepadRightTrigger;
static IMP gOrigGamepadDpad;
static IMP gOrigMicroButtonA;
static IMP gOrigMicroButtonX;
static IMP gOrigMicroDpad;
static IMP gOrigDpadUp;
static IMP gOrigDpadDown;
static IMP gOrigDpadLeft;
static IMP gOrigDpadRight;
...
static BOOL JCFPatchOneJSONFile(NSString *path, BOOL forceRoot) {
    ...
    if (!object) {
        ...
        NSLog(@"[JoyConBridge] DPAD-BRIDGE config missing path=%@", path);
        return NO;
    }
    ...
    if (!changed) {
        ...
        NSLog(@"[JoyConBridge] DPAD-BRIDGE config unchanged path=%@", path);
        return YES;
    }

    BOOL ok = JCFWriteJSONToFile(object, path);
    NSLog(@"[JoyConNintendoOff] patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    NSLog(@"[JoyConBridge] DPAD-BRIDGE config patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    return YES;
}
...
static void JCFPatchMeloNXConfigFiles(void) {
    ...
    NSLog(@"[JoyConBridge] DPAD-BRIDGE config attempt=%lu home=%@", (unsigned long)gJCFConfigPatchAttempts, home);
    ...
    if (!gJCFConfigPatchSucceeded) {
        ...
        NSLog(@"[JoyConBridge] DPAD-BRIDGE config retry_pending");
    }
}
...
static id JCFPhysicalButtons(id self, SEL _cmd) {
    ...
    static BOOL loggedPatchTrigger;
    if (!loggedPatchTrigger) {
        loggedPatchTrigger = YES;
        NSLog(@"[JoyConBridge] DPAD-BRIDGE config patch triggered from physical buttons");
    }
...
static id JCFMappedDpadButton(id dpad, NSString *key, IMP original, SEL selector) {
    NSDictionary *buttons = objc_getAssociatedObject(dpad, kJCFButtonMapKey);
    id button = [buttons objectForKey:key];
    if (button) {
        return button;
    }
    return original ? ((id (*)(id, SEL))original)(dpad, selector) : nil;
}
...
static id JCFGamepadLeftTrigger(id self, SEL _cmd) {
    return JCFMappedButton(self, @"Left Shoulder", gOrigGamepadLeftTrigger, _cmd);
}

static id JCFGamepadRightTrigger(id self, SEL _cmd) {
    return JCFMappedButton(self, @"Right Shoulder", gOrigGamepadRightTrigger, _cmd);
}

static id JCFDpadUp(id self, SEL _cmd) {
    return JCFMappedDpadButton(self, @"Button X", gOrigDpadUp, _cmd);
}

static id JCFDpadDown(id self, SEL _cmd) {
    return JCFMappedDpadButton(self, @"Button B", gOrigDpadDown, _cmd);
}

static id JCFDpadLeft(id self, SEL _cmd) {
    return JCFMappedDpadButton(self, @"Button Y", gOrigDpadLeft, _cmd);
}

static id JCFDpadRight(id self, SEL _cmd) {
    return JCFMappedDpadButton(self, @"Button A", gOrigDpadRight, _cmd);
}
...
static void JCFHookDpadClass(Class cls) {
    ...
    JCFSwizzle(cls, @selector(up), (IMP)JCFDpadUp, &gOrigDpadUp);
    JCFSwizzle(cls, @selector(down), (IMP)JCFDpadDown, &gOrigDpadDown);
    JCFSwizzle(cls, @selector(left), (IMP)JCFDpadLeft, &gOrigDpadLeft);
    JCFSwizzle(cls, @selector(right), (IMP)JCFDpadRight, &gOrigDpadRight);
}

static id JCFGamepadDpad(id self, SEL _cmd) {
    id dpad = gOrigGamepadDpad ? ((id (*)(id, SEL))gOrigGamepadDpad)(self, _cmd) : nil;
    NSDictionary *buttons = objc_getAssociatedObject(self, kJCFButtonMapKey);
    if (dpad && buttons) {
        objc_setAssociatedObject(dpad, kJCFButtonMapKey, buttons, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        JCFHookDpadClass(object_getClass(dpad));

        static BOOL logged;
        if (!logged) {
            logged = YES;
            NSLog(@"[JoyConBridge] DPAD-BRIDGE bridged_dpad A=%p B=%p X=%p Y=%p",
...
static id JCFMicroDpad(id self, SEL _cmd) {
    id dpad = gOrigMicroDpad ? ((id (*)(id, SEL))gOrigMicroDpad)(self, _cmd) : nil;
    NSDictionary *buttons = objc_getAssociatedObject(self, kJCFButtonMapKey);
    if (dpad && buttons) {
        objc_setAssociatedObject(dpad, kJCFButtonMapKey, buttons, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        JCFHookDpadClass(object_getClass(dpad));
        ...
        if (!logged) {
            logged = YES;
            NSLog(@"[JoyConBridge] DPAD-BRIDGE bridged_micro_dpad A=%p B=%p X=%p Y=%p",
                  [buttons objectForKey:@"Button A"],
                  [buttons objectForKey:@"Button B"],
                  [buttons objectForKey:@"Button X"],
                  [buttons objectForKey:@"Button Y"]);
...
static void JCFHookGamepadClass(Class cls) {
    ...
    JCFSwizzle(cls, NSSelectorFromString(@"leftTrigger"), (IMP)JCFGamepadLeftTrigger, &gOrigGamepadLeftTrigger);
    JCFSwizzle(cls, NSSelectorFromString(@"rightTrigger"), (IMP)JCFGamepadRightTrigger, &gOrigGamepadRightTrigger);
    JCFSwizzle(cls, @selector(dpad), (IMP)JCFGamepadDpad, &gOrigGamepadDpad);
}
...
static void JCFBridgeProfileForController(id controller, id profile, NSString *kind) {
    ...
    static BOOL loggedProfilePatchTrigger;
    if (!loggedProfilePatchTrigger) {
        loggedProfilePatchTrigger = YES;
        NSLog(@"[JoyConBridge] DPAD-BRIDGE config patch triggered from profile bridge kind=%@", kind);
    }
    JCFPatchMeloNXConfigFiles();
    objc_setAssociatedObject(profile, kJCFButtonMapKey, buttons, OBJC_ASSOCIATION_RETAIN_NONATOMIC);

    BOOL micro = [kind isEqualToString:@"micro"];
    if (micro) {
    ...
    } else if ([kind isEqualToString:@"extended"] && !loggedExtended) {
        loggedExtended = YES;
        NSLog(@"[JoyConBridge] DPAD-BRIDGE bridged_extended A=%p B=%p X=%p Y=%p L=%p R=%p",
              [buttons objectForKey:@"Button A"],
              [buttons objectForKey:@"Button B"],
              [buttons objectForKey:@"Button X"],
              [buttons objectForKey:@"Button Y"],
...
static id JCFControllerGamepad(id self, SEL _cmd) {
    ...
    JCFBridgeProfileForController(self, profile, @"gamepad");
    return profile;
}
...
static id JCFControllerMicroGamepad(id self, SEL _cmd) {
    ...
    JCFBridgeProfileForController(self, profile, @"micro");
    return profile;
}

static id JCFControllerExtendedGamepad(id self, SEL _cmd) {
    id profile = gOrigControllerExtendedGamepad ? ((id (*)(id, SEL))gOrigControllerExtendedGamepad)(self, _cmd) : nil;
    JCFBridgeProfileForController(self, profile, @"extended");
    return profile;
}
...
static void JCFInstall(void) {
    @autoreleasepool {
        NSLog(@"[JoyConBridge] DPAD-BRIDGE 2026-06-29-dpad-buttons");
        NSLog(@"[JoyConNintendoOff] LOAD MARKER 2026-06-29-dpad-bridge");
        ...
        JCFSwizzle(GCController.class, NSSelectorFromString(@"extendedGamepad"), (IMP)JCFControllerExtendedGamepad, &gOrigControllerExtendedGamepad);

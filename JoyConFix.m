...
static BOOL gJCFConfigPatchDone;
...
static void JCFPatchOneJSONFile(NSString *path, BOOL forceRoot) {
...
static void JCFPatchMeloNXConfigFiles(void) {
    if (gJCFConfigPatchDone) {
        return;
    }
    gJCFConfigPatchDone = YES;
    ...
    NSLog(@"[JoyConNintendoOff] start home=%@", home);
    JCFPatchOneJSONFile([home stringByAppendingPathComponent:@"Documents/config.json"], YES);
    JCFPatchOneJSONFile([home stringByAppendingPathComponent:@"Documents/config-pergame.json"], NO);
}
...
static void JCFInstall(void) {
    @autoreleasepool {
        NSLog(@"[JoyConNintendoOff] LOAD MARKER 2026-06-28-nintendoinput-off");

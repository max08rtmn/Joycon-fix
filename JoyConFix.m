...
static void JCFHookGetterOnClass(Class cls, SEL selector) {
    if (!cls || !selector) {
        return;
    }
    NSString *key = JCFGetterKey(cls, selector);
    ...
    gGetterOriginals[key] = [NSValue valueWithPointer:original];
    NSLog(@"[JoyConFixEarlyGetter] early hooked getter %@ on %@ added=%d",
          NSStringFromSelector(selector), NSStringFromClass(cls), added);
}

static void JCFHookKnownGamepadClassesEarly(void) {
    NSArray<Class> *classes = @[
        NSClassFromString(@"GCGamepad") ?: Nil,
        NSClassFromString(@"GCExtendedGamepad") ?: Nil,
        NSClassFromString(@"GCMicroGamepad") ?: Nil
    ];
    SEL selectors[] = {
        @selector(buttonA),
        @selector(buttonB),
    ...
    for (Class cls in classes) {
        ...
        for (NSUInteger i = 0; i < sizeof(selectors) / sizeof(selectors[0]); i++) {
            JCFHookGetterOnClass(cls, selectors[i]);
        }
    }
}
...
static void JCFLogStatus(NSString *phase) {
    NSLog(@"[JoyConFixEarlyGetter] %@ active version=2026-06-30 early-getter valueHook=%p pressedHook=%p controllers=%lu",
          phase,
...
static void JCFInstallControllerConnectObserver(void) {
    [[NSNotificationCenter defaultCenter] addObserverForName:GCControllerDidConnectNotification
                                                      object:nil
                                                       queue:[NSOperationQueue mainQueue]
                                                  usingBlock:^(NSNotification *note) {
        GCController *controller = [note.object isKindOfClass:GCController.class] ? note.object : nil;
        NSLog(@"[JoyConFixEarlyGetter] controller connected %@ category=%@",
              JCFString(controller.vendorName),
              JCFString([controller respondsToSelector:@selector(productCategory)] ? JCFCall0(controller, @selector(productCategory)) : nil));
        JCFHookKnownGamepadClassesEarly();
        if (controller) {
            JCFHookProfileGetters(controller);
        }
        JCFRefreshMappings();
...
static void JCFInit(void) {
    @autoreleasepool {
        NSLog(@"[JoyConFixEarlyGetter] loaded 2026-06-30 early-getter-reroute");
        ...
        JCFWriteLoadProof();
        JCFHookKnownGamepadClassesEarly();
        JCFInstallControllerConnectObserver();

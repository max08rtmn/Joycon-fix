...
static NSString *JCFHexPreview(NSData *data) {
    const unsigned char *bytes = (const unsigned char *)data.bytes;
    NSUInteger length = MIN(data.length, 256);
...
static NSString *JCFSanitizedDataText(NSData *data) {
    if (!data.length) {
        return @"";
    }
    ...
    if (!text) {
        return [NSString stringWithFormat:@"<hex %@>", JCFHexPreview(data)];
    }
    ...
    for (NSUInteger i = 0; i < text.length; i++) {
        ...
        if (c == '\n' || c == '\r' || c == '\t') {
            [clean appendString:@" "];
        } else if ([controls characterIsMember:c]) {
            [clean appendString:@"."];
        } else {
    ...
    if (clean.length > 3500) {
        return [[clean substringToIndex:3500] stringByAppendingString:@" ...<truncated>"];
    }

    return clean;
}

static void JCFDumpFileAtPath(NSString *path, NSString *label) {
    NSData *data = [NSData dataWithContentsOfFile:path options:0 error:nil];
    if (!data.length) {
        NSLog(@"[JoyConConfigDump] %@ missing_or_empty path=%@", label, path);
        return;
    }

    id plist = [NSPropertyListSerialization propertyListWithData:data options:NSPropertyListImmutable format:nil error:nil];
    if (plist) {
        NSLog(@"[JoyConConfigDump] %@ plist path=%@ value=%@", label, path, plist);
        return;
    }

    NSLog(@"[JoyConConfigDump] %@ size=%lu path=%@ text=%@",
          label,
          (unsigned long)data.length,
          path,
          JCFSanitizedDataText(data));
}

static void JCFDumpKnownConfigFiles(NSString *home) {
    NSArray *relativePaths = @[
        @"Documents/config.json",
        @"Documents/config-pergame.json",
        @"Documents/controller_layouts/default.json",
        @"Documents/system/Profiles.json",
        @"Library/Preferences/com.stossy11.MeloNX.plist",
        @"Documents/bis/user/save/0000000000000013/0/ControlBinding_Joycon_Gameplay.xml",
        @"Documents/bis/user/save/0000000000000013/0/ControlBinding_Joycon_UI.xml",
        @"Documents/bis/user/save/0000000000000013/1/ControlBinding_Joycon_Gameplay.xml",
        @"Documents/bis/user/save/0000000000000013/1/ControlBinding_Joycon_UI.xml"
    ];
    ...
    for (NSString *relativePath in relativePaths) {
        ...
        JCFDumpFileAtPath(path, [NSString stringWithFormat:@"%lu %@", (unsigned long)index, relativePath]);
    }
}

static BOOL JCFPathLooksConfigLike(NSString *relativePath) {
    ...
    NSArray *extensions = @[@".json", @".plist", @".xml", @".ini", @".conf", @".config", @".cfg", @".txt"];
    for (NSString *extension in extensions) {
        if ([lower hasSuffix:extension]) {
            return YES;
        }
    }
...
static void JCFScanConfigFiles(void) {
    ...
    for (NSString *relativePath in enumerator) {
        ...
        if (listedCount < 120 && JCFPathLooksConfigLike(relativePath)) {
            NSDictionary *attributes = [fm attributesOfItemAtPath:path error:nil];
            unsigned long long size = [[attributes objectForKey:NSFileSize] unsignedLongLongValue];
            listedCount++;
            NSLog(@"[JoyConFileList] %lu size=%llu path=%@", (unsigned long)listedCount, size, path);
        }

        JCFScanOneFile(path, &hitCount);
    }

    NSLog(@"[JoyConConfigScan] done checked=%lu hits=%lu listed=%lu", (unsigned long)checkedCount, (unsigned long)hitCount, (unsigned long)listedCount);
}
...
static void JCFInstall(void) {
    @autoreleasepool {
        ...
        NSLog(@"[JoyConConfigScan] LOAD MARKER 2026-06-28-config-dump");

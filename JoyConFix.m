...
static BOOL JCFPatchOneJSONFile(NSString *path, BOOL forceRoot) {
    ...
    if (!object) {
        ...
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config missing path=%@", path);
        return NO;
    }
    ...
    if (!changed) {
        ...
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config unchanged path=%@", path);
        return YES;
    }

    BOOL ok = JCFWriteJSONToFile(object, path);
    NSLog(@"[JoyConNintendoOff] patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config patched=%@ path=%@", ok ? @"YES" : @"NO", path);
    return YES;
}
...
static void JCFPatchMeloNXConfigFiles(void) {
    ...
    NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config attempt=%lu home=%@", (unsigned long)gJCFConfigPatchAttempts, home);
    ...
    if (!gJCFConfigPatchSucceeded) {
        ...
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config retry_pending");
    }
}
...
static id JCFPhysicalButtons(id self, SEL _cmd) {
    ...
    if (!loggedPatchTrigger) {
        ...
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config patch triggered from physical buttons");
    }
...
static void JCFBridgeProfileForController(id controller, id profile, NSString *kind) {
    ...
    if (!loggedProfilePatchTrigger) {
        ...
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE config patch triggered from profile bridge kind=%@", kind);
    }
    JCFPatchMeloNXConfigFiles();
    ...
    if (micro && !loggedMicro) {
        ...
        loggedExtended = YES;
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE bridged_extended A=%p B=%p X=%p Y=%p L=%p R=%p",
              [buttons objectForKey:@"Button A"],
              [buttons objectForKey:@"Button B"],
              [buttons objectForKey:@"Button X"],
              [buttons objectForKey:@"Button Y"],
...
static void JCFInstall(void) {
    @autoreleasepool {
        NSLog(@"[JoyConBridge] EXTENDED-BRIDGE 2026-06-29-extended-gamepad");
        NSLog(@"[JoyConNintendoOff] LOAD MARKER 2026-06-29-extended-bridge");
